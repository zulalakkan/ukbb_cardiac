...
if __name__ == '__main__':
    ...
    orig_path = '/vol/vipdata/data/biobank/cardiac/Application_2964/data'

    # Path to the prepared data
    data_path = '/vol/medic02/users/wbai/data/cardiac_atlas/UKBB_2964/data'

    # Where the csv spreadsheet is saved
    csv_path = '/vol/medic02/users/wbai/data/cardiac_atlas/UKBB_2964/csv'
    ...
    #                 os.mkdir(data_dir)
    #
    #             for seq in ['la_2ch', 'la_4ch']:
    #                 if os.path.exists('{0}/label_up_{1}.nii.gz'.format(orig_dir, seq)):
    #                     nim = nib.load('{0}/label_up_{1}.nii.gz'.format(orig_dir, seq))
    #                     label = nim.get_data()
    #                     label2 = (label == 4) * 1 + (label == 5) * 2
    #                     nim2 = nib.Nifti1Image(label2.astype(np.int16), nim.affine)
    #                     nim2.header['pixdim'][4] = nim.header['pixdim'][4]
    #                     nim2.header['sform_code'] = 1
    #                     nib.save(nim2, '{0}/label_up_{1}.nii.gz'.format(orig_dir, seq))
