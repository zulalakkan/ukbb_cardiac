    ...
    for eid in sorted(os.listdir(data_path)):
        ...
        for seq in ['sa', 'la_2ch', 'la_4ch']:
            ...
            if os.path.exists(label_name):
                ...
                proj_t = np.sum(lab, axis=(0, 1, 2))
                if proj_t[0] == 0:
                    print('  Error at {0}: no annotation at ED frame.'.format(label_name))
                    continue
                if len(np.nonzero(proj_t)[0]) != 2:
                    print('  Error at {0}: number of time frames annotated = {1}, '
                          'not equal to 2.'.format(label_name, len(np.nonzero(proj_t)[0])))
                    continue

                # The ED and ES time frames
                fr = {}
                fr['ED'] = np.nonzero(proj_t)[0][0]
                fr['ES'] = np.nonzero(proj_t)[0][1]
    ...
    for seq in ['sa', 'la_2ch', 'la_4ch']:
        ...
        for eid in sorted(os.listdir(data_path)):
            ...
            for fr in ['ED', 'ES']:
                ...
                # Check whether the annotation contains all the labels
                label = nib.load(label_name).get_data()
                if seq == 'sa':
                    if not np.array_equal(np.unique(label), [0, 1, 2, 3]):
                        flag_good = False
                        break
                elif seq == 'la_2ch':
                    if not np.array_equal(np.unique(label), [0, 1]):
                        flag_good = False
                        break
                elif seq == 'la_4ch':
                    if not np.array_equal(np.unique(label), [0, 1, 2]):
                        flag_good = False
                        break
        ...
        bad_data = pd.read_csv(os.path.join(csv_path, 'bad_data_{0}.csv'.format(seq)))['eid'].tolist()
        ...
        df.to_csv(os.path.join(csv_path, 'good_data_{0}.csv'.format(seq)), index=None)

    """
        Step 4: Split the dataset into training, validation and testing sets.
        """
    for seq in ['sa', 'la_2ch', 'la_4ch']:
        data_list = pd.read_csv(os.path.join(csv_path, 'good_data_{0}.csv'.format(seq)))['eid'].tolist()
