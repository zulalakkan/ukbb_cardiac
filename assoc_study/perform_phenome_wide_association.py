...
from ukb_field_categories import *
...
# # # # # # # # # # # # # # # # # # # #
# Step 1: read IDP data
# # # # # # # # # # # # # # # # # # # #
# Image derived phenotypes
data_path = '/vol/vipdata/data/biobank/cardiac/Application_18545/clinical'
df_idp = pd.read_csv('{0}/clinical_measures_26k_collated_qced.csv'.format(data_path), index_col=0)
print(df_idp.shape)

# # # # # # # # # # # # # # # # # # # #
# Step 2: read life-style data
# # # # # # # # # # # # # # # # # # # #
# Participant characteristics
info_path = '/vol/vipdata/data/biobank/cardiac/Application_18545/downloaded'
...
field_names = {}
ukb_in = '/vol/vipdata/data/biobank/cardiac/Application_18545/downloaded/ukb_field_added.txt'
...
# Age provided by UK Biobank seems to be floored, i.e. with half a year error.
# To get more accurate age values, we calculate age by date.
# age = df_info['Age when attended assessment centre', '21003-2.0'].values
age = np.zeros(len(df))
for i in range(len(df)):
    # Calculate age
    d1 = datetime.date(df.iloc[i]['Year of birth', '34-0.0'],
                       df.iloc[i]['Month of birth', '52-0.0'],
                       15)
...
df_conf.to_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/confounders_26k.csv')
...
# Step 4.3: normalise non-IDPs
df_cont = pd.read_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/continuous.csv', index_col=0)
...
for i in range(n_col):
    ...
    # if field_id in df_cont.index:
    is_continuous = df_cont.loc[field_id]['continuous']
    # else:
    #     print('{},{}'.format(field_id, field_names[field_id]))
    #     is_continuous = False

    if is_continuous:
        # If it is a continuous variable
        # Perform standard normalisation
        x = normalise(x)
    else:
        # If we are not sure whether it is a continuous or categorical variable,
        # first convert it into a continuous variable using rank-based inverse
        # normal transform
        x = rank_normalise(x)
    df.iloc[:, i][valid_idx] = x
df.to_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/normalised_non_IDPs_26k.csv')
...
df_idp.to_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/normalised_IDPs_26k.csv')

df_beta = pd.DataFrame(beta,
                       index=['sex', 'age', 'sex * age', 'weight', 'height'],
                       columns=df_idp.columns)
df_beta.to_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/beta_IDPs_26k.csv')

df = pd.read_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/normalised_non_IDPs_26k.csv',
                 header=[0, 1], index_col=0)
df_idp = pd.read_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/normalised_IDPs_26k.csv',
...
df_corr.to_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/table_corr_26k.csv')
df_p.to_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/table_p_26k.csv')
df_log_p.to_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/table_log_p_26k.csv')

df_corr = pd.read_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/table_corr_26k.csv',
                      header=[0, 1], index_col=0)
df_p = pd.read_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/table_p_26k.csv',
                   header=[0, 1], index_col=0)
df_log_p = pd.read_csv('/vol/vipdata/data/biobank/cardiac/Application_18545/clinical/table_log_p_26k.csv',
                        header=[0, 1], index_col=0)
corr = df_corr.values
corr_p = df_p.values
...
# plt.plot([0, N], [-math.log10(p_fdr), -math.log10(p_fdr)], 'k--')
# plt.text(-1, -math.log10(p_fdr), 'FDR', horizontalalignment='right', fontsize=11)
...
# Correlation plot
plt.figure()
ax = sns.scatterplot(x='x', y='corr', hue='Anatomy', size='log_p',
                     sizes=(20, 200), size_norm=mpl.colors.Normalize(vmin=0, vmax=300),
                     data=df_table, alpha=0.8)
...
plt.xticks(xticks, xticklabels, fontsize=10)
plt.yticks(fontsize=12)
plt.xlabel('Lifestyle and health information', fontsize=12, fontweight='bold')
